...
*/
#[map]
static mut PROCESS_PATHS: PerfMap<[c_char;16]> = PerfMap::with_max_entries(1024);
#[kprobe]
fn syscall__execve(regs: Registers) {
    let mut path = OpenPath::default();
    unsafe {
        let filename = regs.parm1() as *const u8;
        let pid = bpf_get_current_pid_tgid() >> 32;
        let comm = bpf_get_current_comm();
        bpf_probe_read()
        bpf_trace_printk(b"kernel mode test\0");
        bpf_get
        PROCESS_PATHS.insert(regs.ctx, &comm);
    }
}
